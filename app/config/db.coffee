...
module.exports =
  mongo:
    if MONGO_DB_USER and MONGO_DB_PASS
      MONGO_DB_URL: process.env.MONGOLAB_URI || process.env.MONGOHQ_URL ||
        "mongodb://#{MONGO_DB_USER}:#{MONGO_DB_PASS}@#{DB_HOST}:#{MONGO_DB_PORT}/#{MONGO_DB_NAME}"
    else
      MONGO_DB_URL: process.env.MONGOLAB_URI || process.env.MONGOHQ_URL ||
        "mongodb://#{DB_HOST}:#{MONGO_DB_PORT}/#{MONGO_DB_NAME}"
  redis:
    # Redis settings
    if redisService?
      cfRedis =
