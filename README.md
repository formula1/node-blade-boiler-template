...
#Background
Node.js Boiler template, written in coffeescript, with user administration interface and with Federated Login, via Passport.
...
# Style Guide
Uses the [CoffeeScript Style Guide](https://github.com/polarmobile/coffeescript-style-guide) with the following exceptions:
...
#Versioning
Maintained under the [Semantic Versioning Guidelines][10] as much as possible. Releases will be numbered with the following format:
...
#Todo
 ...
 - `[ ]` [Vagrant][13] - create a dev environment with Riak cluster and and use the Chef provisioning tool to:
    - `[ ]` install packages, node.js, express etc... through npm
